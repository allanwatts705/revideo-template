...
import { createRef, makeProject, waitFor, ThreadGeneratorFactory, Reference, ThreadGenerator, all, createSignal } from "@revideo/core";
...
const scene = makeScene2D('scene', function* (view: View2D): ThreadGenerator {
  ...
  const dingRef = createRef<Audio>();
  const tickRef = createRef<Audio>();
  const applauseRef = createRef<Audio>();
  ...
  view.add(
    ...
    >
      ...
      <Audio ref={dingRef} play={false} src={'/ding.mp3'} />
      <Audio ref={tickRef} play={false} src={'/ticking.mp3'} />
      <Audio ref={applauseRef} play={false} src={'/applause.mp3'} />
      {/* Question Section */}
  ...
  // Animation sequence
  yield waitFor(sfxRef().getDuration());
  ...
  for (let i = 0; i < cardRefs.length; i++) {
    yield*  cardRefs[i]().reveal()
     yield* waitFor(cardRefs[i]().ttsDuration())
  }
